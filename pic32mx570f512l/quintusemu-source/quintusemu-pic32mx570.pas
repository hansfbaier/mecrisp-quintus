...
begin
  case addr of
   ...
   $BF886320: begin // Catch bit-bang terminal code for receiving characters upon read from PORTD register.
                read32 := 0;
                write_register(8, key);

                // pop x1
                write_register(31, read32(read_register(2)));
                write_register(2, read_register(2) + 4);
              end;
...
var first_portd_access : boolean = true;
...
begin
  ...
  case addr of
    ...
    $BF886320: begin // Catch bit-bang terminal code for emitting characters upon write to PORTD register.

                 if first_portd_access then first_portd_access := false
                 else
                 begin
                   emit(read_register(8) and $FF);

                   write_register(8, read32(read_register(9))); // drop
                   write_register(9, read_register(9) + 4);

                   write_register(2, read_register(2) + 4); // rdrop
                 end;
               end;
