...
begin
  ...
  case addr of
    $40020000: read32 := $100;        // UART Flags Register
    $40010000: read32 := key;         // UART Data Register
  else
...
begin
  ...
  case addr of
    ...
    $DABBAD00: begin // Special helper to generate binaries with precompiled sources.
                 ...
                 filewrite(coredump, memory[$00000000], data - $00000000);
                 fileclose(coredump);
                 halt;
               end;

    $40010000: emit(data and $FF);  // UART Data Register
  else
    memory[addr  ] :=  data         and $FF;
    memory[addr+1] := (data shr  8) and $FF;
...
begin
  ...
  writeln;
  addr := $00000000;
  while (addr < $00000000 + binarysize) do
  begin
    inst := read32(addr);
...
begin
  ...
  fileread(binaryimage, memory[$00000000], binarysize);
