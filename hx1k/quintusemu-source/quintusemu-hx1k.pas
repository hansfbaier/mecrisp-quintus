...
begin
  ...
  case addr of
    ...
    $DABBAD00: begin // Special helper to generate binaries with precompiled sources.
                 ...
                 filewrite(coredump, memory[$00810000], data - $00810000);
                 fileclose(coredump);
                 halt;
               end;

    $00400008: emit(data and $FF);  // UART Data Register
  else
    memory[addr  ] :=  data         and $FF;
    memory[addr+1] := (data shr  8) and $FF;
...
begin
  ...
  writeln;
  addr := $00810000;
  while (addr < $00810000 + binarysize) do
  begin
    inst := read32(addr);
...
begin
  ...
  fileread(binaryimage, memory[$00810000], binarysize);
