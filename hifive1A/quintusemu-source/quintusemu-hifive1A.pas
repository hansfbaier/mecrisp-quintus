...
var memory_flash : array[$20400000..$20410000] of byte;
    memory_ram   : array[$00000000..$00004000] of byte;

function flash(addr : dword) : boolean;
begin
  flash := ($20400000 <= addr) and (addr < $20410000);
end;

function ram(addr : dword) : boolean;
begin
  ram := ($80000000 <= addr) and (addr < $80004000);
end;
...
begin
  ...
  if   ram(addr) then read8 :=   memory_ram[addr - $80000000];
end;
...
begin
  ...
  case addr of
    ...
    $10008004: read32 := $C0000000; // PRCI_HFXOSCCFG: Oscillator up and running
    $10013000: read32 := 0;   // UART_TXDATA: Transmit FIFO is never full.
    $10013004: read32 := key; // UART_RXDATA: Character always available.
    ...
    if   ram(addr) then read32 :=   memory_ram[addr     - $80000000] or
                                    memory_ram[addr + 1 - $80000000] shl  8  or
                                    memory_ram[addr + 2 - $80000000] shl 16  or
                                    memory_ram[addr + 3 - $80000000] shl 24;
  end;
end;
...
begin
  ...
  if   ram(addr) then   memory_ram[addr - $80000000] := data and $FF;
end;
...
begin
  ...
  case addr of
    ...
    $DABBAD00: begin // Special helper to generate binaries with precompiled sources.
                 ...
                 filewrite(coredump, memory_flash[$20400000], data-$20400000);  
                 fileclose(coredump);
                 halt;
               end;
    ...
    if   ram(addr) then begin
                            memory_ram[addr  -$80000000] :=  data         and $FF;
                            memory_ram[addr+1-$80000000] := (data shr  8) and $FF;
                            memory_ram[addr+2-$80000000] := (data shr 16) and $FF;
                            memory_ram[addr+3-$80000000] := (data shr 24) and $FF;
                        end;
  end;
end;
...
begin
  ...
  fileread(binaryimage, memory_flash[$20400000], binarysize);
