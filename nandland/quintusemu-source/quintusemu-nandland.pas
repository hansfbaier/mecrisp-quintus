...
var memory : array[0..$00040000] of byte;
...
begin
  ...
  case addr of
    $00010010: read32 := $100;        // UART Flags Register
    $00010008: read32 := key;         // UART Data Register
  else
...
begin
  ...
  case addr of
    ...
    $DABBAD00: begin // Special helper to generate binaries with precompiled sources.
                 ...
                 filewrite(coredump, memory[$00030000], data - $00030000);
                 fileclose(coredump);
                 halt;
               end;

    $00010008: emit(data and $FF);  // UART Data Register
  else
    memory[addr  ] :=  data         and $FF;
    memory[addr+1] := (data shr  8) and $FF;
    memory[addr+2] := (data shr 16) and $FF;
    memory[addr+3] := (data shr 24) and $FF;
  end;
end;
...
function printopcode : string;
begin
  if (read16(get_pc) and 3) < 3 then printopcode :=  word2hex(read16(get_pc)) + '    '
                                else printopcode := dword2hex(read32(get_pc)) ;
end;
...
begin
  ...
  writeln;
  addr := $00030000;
  while (addr < $00030000 + binarysize) do
  begin
    inst := read32(addr);
...
begin
  ...
  fillchar(memory, sizeof(memory), 255);
  ...
  fileread(binaryimage, memory[$00030000], binarysize);
 ...
 // begin
  repeat
    ...
    // writeregister;
    //writeln(dword2hex(get_pc), ' : ', printopcode, '  ',  disassemble(get_pc, read32(get_pc)) );
    // writeln;
    execute;
  until false;
  //end;
